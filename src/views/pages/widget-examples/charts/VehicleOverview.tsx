        ...
        <div className='flex flex-col gap-6'>
          <div className='flex is-full'>
            {data.map((item, index) => (
              <div
                key={index}
                className={classnames(item.widthClass, 'flex flex-col gap-[38px] relative overflow-hidden')}
              >
                <Typography className='relative max-sm:hidden text-sm font-medium text-textSecondary'>{item.heading}</Typography>
                <LinearProgress
                  variant='determinate'
                  value={-1}
