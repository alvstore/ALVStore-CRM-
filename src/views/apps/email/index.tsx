...
const EmailWrapper = ({ folder, label }: { folder?: string; label?: string }) => {
  ...
  const emailStore = useSelector((state: RootState) => state.email)
  ...
  // Vars
  const uniqueLabels = emailStore?.emails ? [...new Set(emailStore.emails.flatMap((email: Email) => email.labels))] : []
